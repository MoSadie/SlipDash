...
import { Crew, emptyCrew } from "./crew";

export default function Home() {
  const searchParams = useSearchParams();
  let defaultUrl = searchParams.has("url") ? searchParams.get("url") : "";

  const [url, setUrl] = useState(defaultUrl);
  const [crewmate, setCrewmate] = useState(emptyCrew);

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white p-4">
      <div className="bg-gray-500 p-4 rounded-md shadow-md">
        <h1 className="text-2xl font-bold">SlipDash</h1>
        <div className="p-5">{step1(url, setUrl)}</div>
...
function step1(url: string | null, setUrl: Function) {
  return (<div>
